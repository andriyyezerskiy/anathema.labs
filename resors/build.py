...
CSS = r"""
    ...
    * { box-sizing: border-box; }
    ...
    .fan-label { display: flex; align-items: center; justify-content: center; gap: 7px;
    ...
    /* ---------- ship: export, sync, conflicts ---------- */
    .ship { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; margin-top: 2.6rem; }
    @media (max-width: 900px) { .ship { grid-template-columns: 1fr; } }
    .ship-card { border: 1px solid var(--rule-2); border-radius: 14px; padding: 1.6rem;
                 background: var(--panel); display: flex; flex-direction: column; }
    .ship-card .cap, .cf-copy .cap { font-family: var(--mono); font-size: 0.64rem; letter-spacing: 0.12em;
                                     text-transform: uppercase; color: var(--ink3); }
    .ship-card h3, .cf-copy h3 { margin: 0.7rem 0 0.5rem; font-size: 1.15rem; }
    .ship-card p, .cf-copy p { margin: 0; color: var(--ink2); font-size: 0.92rem; }
    ...
    /* export: one action writes the whole catalog */
    .ex { margin-top: auto; padding-top: 1.4rem; }
    .ex-box { border: 1px solid var(--rule-2); border-radius: 11px; overflow: hidden; }
    .ex-hd { display: flex; align-items: center; gap: 8px; padding: 8px 10px;
             border-bottom: 1px solid var(--rule-2); font-family: var(--mono); font-size: 10px;
             letter-spacing: 0.04em; color: var(--ink3); }
    .ex-chip { display: inline-flex; align-items: center; gap: 6px; padding: 3px 9px;
               border: 1px solid var(--rule); border-radius: 999px; color: var(--ink); }
    .ex-chip svg { width: 11px; height: 11px; flex: none; opacity: 0.8; }
    .ex-go { margin-left: auto; background: var(--ink); color: var(--paper);
             border-radius: 6px; padding: 3px 9px; letter-spacing: 0.06em; }
    .ex-tree { margin: 0; padding: 9px 11px; list-style: none;
               font-family: var(--mono); font-size: 11px; line-height: 1.75; color: var(--ink2); }
    .ex-tree li { display: flex; gap: 10px; align-items: baseline; }
    .ex-tree li.d { color: var(--ink); }
    .ex-tree li.f { padding-left: 15px; }
    .ex-tree li.m { padding-left: 15px; color: var(--ink3); }
    .ex-tree em { font-style: normal; font-size: 10px; color: var(--ink3); margin-left: auto; }

    /* sync: a live wire, both directions */
    .sync-wire { margin-top: 1.5rem; display: grid; grid-template-columns: auto 1fr auto;
                 gap: 12px; align-items: center; }
    @media (max-width: 420px) { .sync-wire { grid-template-columns: 1fr; } }
    .sw-end { border: 1px solid var(--rule); border-radius: 10px; padding: 0.5rem 0.62rem;
              text-align: center; min-width: 96px; }
    .sw-end b { display: block; font-size: 12.5px; font-weight: 600; letter-spacing: -0.01em; }
    .sw-end em { display: block; font-style: normal; font-family: var(--mono); font-size: 9.5px;
                 letter-spacing: 0.08em; text-transform: uppercase; color: var(--ink3); margin-top: 3px; }
    .sw-lanes { display: grid; gap: 13px; min-width: 0; }
    .sw-lane { position: relative; height: 20px; }
    .sw-lane::before { content: ""; position: absolute; left: 0; right: 0; top: 50%;
                       height: 1px; background: var(--rule); }
    .sw-lane b { position: absolute; left: 50%; top: 50%; transform: translate(-50%, -50%);
                 background: var(--panel); padding: 0 8px; white-space: nowrap;
                 font-family: var(--mono); font-weight: 400; font-size: 9.5px;
                 letter-spacing: 0.09em; text-transform: uppercase; color: var(--ink3); }
    .sw-lane i { position: absolute; top: 50%; margin-top: -3px; width: 6px; height: 6px;
                 border-radius: 50%; background: var(--accent); }
    .sw-out i { animation: swR 2.8s linear infinite; }
    .sw-in i { animation: swL 2.8s linear infinite; }
    @keyframes swR { 0% { left: 0; opacity: 0 } 12% { opacity: 1 }
                     88% { opacity: 1 } 100% { left: calc(100% - 6px); opacity: 0 } }
    @keyframes swL { 0% { left: calc(100% - 6px); opacity: 0 } 12% { opacity: 1 }
                     88% { opacity: 1 } 100% { left: 0; opacity: 0 } }
    @media (prefers-reduced-motion: reduce) { .sw-out i, .sw-in i { animation: none; opacity: 0 } }
    .sw-note { display: flex; flex-wrap: wrap; gap: 7px; margin-top: auto; padding-top: 1.4rem; }
    .sw-note span { font-family: var(--mono); font-size: 10px; color: var(--ink2);
                    border: 1px solid var(--rule-2); border-radius: 999px; padding: 4px 9px; }

    /* conflicts: the rule decides, the sync keeps running */
    .cf { margin-top: 20px; border: 1px solid var(--rule-2); border-radius: 14px; background: var(--panel);
          padding: 1.6rem; display: grid; grid-template-columns: 1fr 1.2fr; gap: 26px; align-items: start; }
    @media (max-width: 900px) { .cf { grid-template-columns: 1fr; gap: 20px; } }
    .cf-conflict { display: grid; grid-template-columns: 1fr 1fr; gap: 10px; }
    @media (max-width: 560px) { .cf-conflict { grid-template-columns: 1fr; } }
    .cf-side { position: relative; display: flex; align-items: center; gap: 9px; padding: 10px 11px;
               border: 1px solid var(--rule-2); border-radius: 10px; background: var(--paper);
               transition: opacity 0.22s ease, border-color 0.22s ease; }
    .cf-side i { width: 24px; height: 24px; border-radius: 6px; flex: none; }
    .cf-side b { display: block; font-size: 12.5px; letter-spacing: -0.01em; }
    .cf-side em { display: block; font-style: normal; font-family: var(--mono);
                  font-size: 10px; color: var(--ink3); margin-top: 2px; }
    .cf-src { margin-left: auto; font-family: var(--mono); font-size: 9.5px; letter-spacing: 0.08em;
              text-transform: uppercase; color: var(--ink3); }
    .cf-kept { display: none; position: absolute; top: -8px; right: 10px; background: var(--accent);
               color: #fff; font-family: var(--mono); font-size: 9px; letter-spacing: 0.08em;
               text-transform: uppercase; border-radius: 999px; padding: 2px 7px; }
    .cf-demo[data-mode="resors"] .cf-a, .cf-demo[data-mode="xcode"] .cf-b,
    .cf-demo[data-mode="newest"] .cf-a { border-color: var(--accent); }
    .cf-demo[data-mode="resors"] .cf-a .cf-kept, .cf-demo[data-mode="xcode"] .cf-b .cf-kept,
    .cf-demo[data-mode="newest"] .cf-a .cf-kept { display: block; }
    .cf-demo[data-mode="resors"] .cf-b, .cf-demo[data-mode="xcode"] .cf-a,
    .cf-demo[data-mode="newest"] .cf-b { opacity: 0.42; }
    .cf-demo[data-mode="ask"] .cf-side { border-style: dashed; }
    .cf-verdict { margin: 0.9rem 0 0; font-size: 0.88rem; color: var(--ink2); min-height: 2.6em; }
    .cf-modes { display: flex; flex-wrap: wrap; gap: 7px; margin-top: 0.9rem; }
    .cf-modes button { cursor: pointer; padding: 0.34rem 0.75rem; border-radius: 999px;
                       border: 1px solid var(--rule); background: transparent; color: var(--ink2);
          ...
          <div class="pane">
            ...
            <div class="dupe-body dupe-body--one">
              ...
              <div class="fanout">
                ...
                <span class="fan-label">Export or sync <span class="tag tag--plus">Resors<b>+</b></span></span>
    ...
    <section class="stage" id="ship">
      <div class="wrap">
        ...
        <div class="stage-copy">
          ...
          <p class="lede">The system is only useful if it lands in the project. Export assets and groups in one action, straight into the asset catalog. Or set up sync between a group and a catalog, and assets import and export on their own, both directions.</p>
        </div>
        <div class="ship">
          <article class="ship-card">
            ...
            <div class="ex" aria-hidden="true">
              <div class="ex-box">
                <div class="ex-hd">
                  <span class="ex-chip">__FOLDER__Studio Core</span>
                  <span>18 assets</span>
                  <span class="ex-go">Export</span>
                </div>
                <ul class="ex-tree">
                  <li class="d">Colors.xcassets</li>
                  <li class="f">Ember.colorset<em>3 appearances</em></li>
                  <li class="f">Ocean.colorset<em>3 appearances</em></li>
                  <li class="f">Waypoint.symbolset<em>margins + scale</em></li>
                  <li class="m">+ 15 more, same pass</li>
                </ul>
              </div>
            </div>
          </article>
          <article class="ship-card">
            ...
            <div class="sync-wire" aria-hidden="true">
              ...
              <div class="sw-lanes">
                <div class="sw-lane sw-out"><b>Edits out</b><i></i></div>
                <div class="sw-lane sw-in"><b>Changes in</b><i></i></div>
              </div>
        ...
        <div class="cf">
          ...
          <div class="cf-demo" data-cf data-mode="newest">
            <div class="cf-conflict">
              <div class="cf-side cf-a">
                <span class="cf-kept">Kept</span>
                <i style="background:#E08A2C"></i>
                <div><b>Ember</b><em>#E08A2C &middot; 14:22</em></div>
                <span class="cf-src">Resors</span>
              </div>
              <div class="cf-side cf-b">
                <span class="cf-kept">Kept</span>
                <i style="background:#D9821F"></i>
                <div><b>Ember</b><em>#D9821F &middot; 09:05</em></div>
                <span class="cf-src">Xcode</span>
              </div>
            </div>
            <p class="cf-verdict" data-cf-verdict>The most recent edit wins, whichever side made it. Here that is Resors, at 14:22.</p>
            <div class="cf-modes" role="group" aria-label="Conflict resolution mode">
...
JS = """
    ...
    (function () {
      ...
      if (!demo) return;
      var out = demo.querySelector('[data-cf-verdict]');
      var btns = demo.querySelectorAll('[data-cf-mode]');
      var says = {
      ...
      btns.forEach(function (b) {
        b.addEventListener('click', function () {
          var m = b.getAttribute('data-cf-mode');
          demo.setAttribute('data-mode', m);
          btns.forEach(function (o) { o.setAttribute('aria-pressed', o === b ? 'true' : 'false'); });
          out.textContent = says[m];
        });
      });
    })();
